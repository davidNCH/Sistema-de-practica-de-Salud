...
go
IF EXISTS(SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS
WHERE TABLE_NAME='boleta') /*libroad = libro adquerdido*/
DROP VIEW boleta
GO
create view boleta as
 ...
 prestamo inner join lector on lector.id_lector=prestamo.id_lector inner join libros on libros.id_libro=prestamo.id_lector
